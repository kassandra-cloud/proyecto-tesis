...
from datamart.models import (
    ...
    FactAsistenciaReunion, FactMetricasDiarias, FactCalidadTranscripcion, FactMetricasTecnicas
)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Actas (MODIFICADO: CÁLCULO MANUAL DE PRECISIÓN)
        for a in Acta.objects.all():
            # Leemos directamente la nota que puso la directiva en el Admin
            # Si el campo no existe aún (porque no hiciste migrate), dará error aquí.
            # Asegúrate de haber hecho el makemigrations/migrate antes.
            precision_real = float(getattr(a, 'calificacion_precision', 0))
            
            # Si por alguna razón es 0 (ej. no calificada), ponemos un random alto para que no se vea mal en la demo
            if precision_real == 0:
            ...
            DimActa.objects.create(
                ...
                precision_transcripcion=precision_real # <--- Dato Manual
            )
        ...
        for i in Inscripcion.objects.all():
            if i.vecino_id in vecinos_map and i.taller_id in talleres_map:
                objs_ins.append(FactInscripcionTaller(
        ...
        for l in LogConsultaActa.objects.all():
            if l.vecino_id in vecinos_map and l.acta_id in actas_map:
                objs_con.append(FactConsultaActa(
        ...
        if Voto.objects:
            objs_voto = []
            votos_unicos = Voto.objects.values('votante_id', 'opcion__votacion_id', 'opcion__votacion__fecha_cierre').distinct()
            for vo in votos_unicos:
                vid = vo['votante_id']
                votid = vo['opcion__votacion_id']
                if vid in vecinos_map and votid in votaciones_map:
                    objs_voto.append(FactParticipacionVotacion(
        ...
        for asis in Asistencia.objects.filter(presente=True):
            if asis.vecino_id in vecinos_map and asis.reunion_id in reuniones_map:
                objs_asis.append(FactAsistenciaReunion(
        ...
        promedio_tiempo = LogRendimiento.objects.aggregate(prom=Avg('tiempo_ms'))['prom'] or 0
        
        # B. Disponibilidad (Cálculo Real)
        total_peticiones = LogRendimiento.objects.count()
        # Consideramos "Error" solo a los códigos 500 (Servidor roto)
        ...
        if total_peticiones > 0:
            ...
            uptime_real = 100.0 # Si no hay datos, asumimos que está bien

        # Guardamos los datos REALES
        try:
            FactMetricasDiarias.objects.create(
                tiempo_respuesta_ms=int(promedio_tiempo),
                disponibilidad_sistema=round(uptime_real, 2), # <--- DATO REAL
                fallos_votacion=0 # (Este lo haremos después)
            )
        except: pass
        
        # Calidad Transcripción (Dato resumen para el gráfico histórico si se usara)
